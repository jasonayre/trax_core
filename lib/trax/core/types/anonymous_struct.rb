      ...
      class AnonymousStruct < ::Hashie::Dash
        ...
        def self.define_attribute_class_for_type(type_name, property_name, *args, coerce:false, **options, &block)
          attribute_klass = if options.key?(:extend)
            ...
            _klass = fields_module.const_set(property_name.to_s.classify, ::Class.new(_klass_prototype, &block))
            _klass
          else
            classified_type_name = _classified_type_name(type_name)
            fields_module.const_set(property_name.to_s.classify, ::Class.new("::Trax::Core::Types::#{classified_type_name}".constantize, &block))
          end
